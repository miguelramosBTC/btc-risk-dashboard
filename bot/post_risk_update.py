...
def header_lines(risk, ddate, hist):
    return (f"{level_emoji(risk)} BTC Risk {risk:.2f} — {level_word(risk)}",
            f"📊 {hist} · as of {fmt_day(ddate)}")
...
def main() -> int:
    ...
    if mode == "long":
        ...
        sweet = min(max(min_w + 120, 350), max_w)
        ...
        if problem and "too short" in problem:
            ...
            if not long_text_ok(body2, min_w):
                ...
                body = body2
                problem = long_text_ok(body, min_w)
        if problem and "too short" in problem and word_count(body) >= 120:
            print(f"[llm] still short ({word_count(body)}w) — posting anyway")
            problem = ""
        if not problem and body:
            text, used_llm = build_long(risk, ddate, hist, body), True
            print(f"[bot] long body: {word_count(body)} words, "
                  f"{len(text)} chars total")
        else:
            print(f"[llm] long mode unusable ({problem or 'no output'}) — "
                  f"degrading to short format")

    if not text:
        text = build_short(risk, ddate, hist, today)

    # URL guard on the post TEXT (image text is exempt and free)
    if re.search(r"https?://|\b[\w-]+\.(?:com|net|org|io|xyz|co|ai)\b", text, re.I):
        print("[bot] FATAL: URL/domain in post text — aborting ($0.20 URL "
              "billing + against design)")
        return 1
    if len(text) > 24000:
        print("[bot] FATAL: text over X's 25k limit after all caps?!")
        return 1
    ...
    print(f"[bot] {'LONG ' + str(word_count(text)) + 'w' if is_long else 'short ' + str(x_len(text)) + '/280'}"
    ...
    extra = {"data date": ddate, "risk": f"{risk:.4f}", "level": level,
             ...
             "words": word_count(text), "LLM": "used" if used_llm else "fallback",
             "gauge": "attached" if media_path else "none"}

    if dry:
        write_summary("DRY-RUN (not posted)", text, extra)
        if media_path:
