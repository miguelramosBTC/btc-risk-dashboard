...
Fonts: tries Space Grotesk (variable, Bold instance) + IBM Plex Mono fetched
at runtime into bot/.fonts; falls back to DejaVu (present on GitHub runners).
If the ₿ glyph is missing in the fallback font, the mark is drawn from a bold
"B" plus the two protruding bars.
"""
import io
import math
import os
...
def _load(kind, size):
    """kind: 'display' or 'mono'. Returns an ImageFont, best available."""
    if kind == "display":
...
def render(risk: float, level: str, hist_line: str, date_str: str,
    ...
    d.text((x0, 615 * SS), f"as of {date_str}", font=_load("mono", 27 * SS),
           fill=(120, 134, 150))
    d.text((w - 60 * SS, h - 58 * SS), "bitcoinrisk.net",
           font=_load("mono", 30 * SS), fill=(232, 200, 74), anchor="rm")
...
if __name__ == "__main__":
    render(0.28, "LOW", "Lower than 78% of the last 365 days",
           "2026-07-06", "/tmp/gauge_sample.png")
