...
/* Older than this and the v3 row is no longer "current": say so and serve v2
   rather than presenting a stale number as today's. Matches v3.js and
   btc-risk-weekly.mjs on purpose. */
export const MAX_STALE_DAYS = 4;

/* The committed v3 row.
   Source is series/v3.0_last90.json rather than the tape itself: write_window()
   copies rows into it VERBATIM, so its last row is byte-identical to the last
   line of series/v3.0.jsonl — and a Worker should not pull a multi-megabyte
   JSONL on every request to read one line. Nothing here recomputes: the score
   is whatever the row says. */
export async function loadV3(context) {
  try {
    const res = await context.env.ASSETS.fetch(
      new URL("/series/v3.0_last90.json", context.request.url));
    if (!res.ok) return { row: null, rows: null, reason: `v3 window ${res.status}` };
    const doc = await res.json();
    const rows = doc && Array.isArray(doc.rows) ? doc.rows : null;
    if (!rows || !rows.length) return { row: null, rows: null, reason: "v3 window carries no rows" };
    const row = rows[rows.length - 1];
    if (!row || typeof row.risk01 !== "number" || !row.asof_date) {
      return { row: null, rows: null, reason: "last v3 row is missing risk01 or asof_date" };
    }
    const lag = Math.round(
      (Date.parse(new Date().toISOString().slice(0, 10) + "T00:00:00Z")
        - Date.parse(row.asof_date + "T00:00:00Z")) / 86400000);
    if (lag > MAX_STALE_DAYS) {
      return { row: null, rows: null,
        reason: `v3 tape is ${lag} days behind (${row.asof_date}), limit ${MAX_STALE_DAYS}` };
    }
    return { row, rows, reason: null, schema_version: doc.schema_version || row.schema_version || "v3.0" };
  } catch (e) {
    return { row: null, rows: null, reason: `v3 window unavailable (${e.message})` };
  }
}
